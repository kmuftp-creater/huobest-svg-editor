...
- 大尺寸自動縮放至畫布（保留 5% 邊距）
- 匯出 SVG（向量）
- **匯出 PNG 含設定對話框（v0.8.0）**：
  - 輸出範圍：整個畫布（1200×800）/ 依內容裁切（合併 bbox + 8px 邊距）
  - 解析度倍率：1× / 2× / 3× / 4× / 自訂寬高（可鎖定比例）
  - 背景：白色 / 透明（保留 alpha 通道）
  - 即時預覽輸出尺寸，檔名含實際尺寸
...
| v0.8.0 | **PNG 匯出自訂尺寸** | 匯出設定對話框：範圍 / 倍率 / 自訂尺寸 / 透明背景 |
