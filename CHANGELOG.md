...
## v0.8.0（當前）
### 新功能：PNG 匯出自訂尺寸
之前 PNG 匯出寫死 1200×800，檔案偏小。新增「匯出 PNG 設定」對話框：

**輸出範圍**
- 整個畫布（1200 × 800）
- 依內容裁切（只輸出有物件的範圍，自動計算合併 bbox + 8px 邊距）

**解析度倍率**
- 1× / 2× / 3× / 4×（以基準尺寸乘算，例如 4× 整個畫布 = 4800 × 3200）
- 自訂：直接輸入寬高（可鎖定比例，改一邊自動算另一邊）

**背景**
- 白色 / 透明（PNG 保留 alpha 通道）

**即時預覽**：對話框內顯示最終輸出尺寸（如「輸出尺寸：4800 × 3200 px」）

檔名含實際尺寸，例如 `design-4800x3200-{timestamp}.png`。

### 技術細節
- `buildExportSvg(opts)` 參數化：支援 region（canvas / content）與 bg（white / transparent），回傳 `{ svg, x, y, w, h }`
- `getContentBBox()` 計算所有物件的合併邊界
- `exportPngWith(settings)` 依目標尺寸放大 canvas 並 `drawImage(img, 0, 0, outW, outH)` 高解析輸出
- 匯出 SVG 與範本縮圖同步更新為新的回傳型別
